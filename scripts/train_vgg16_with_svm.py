...
from eye.models.vgg16_with_svm import Vgg16
...
# python scripts/train_vgg16_with_svm.py --batch=2 --epoch=1 --patience=5 --loss=binary_crossentropy --data=./Data --result=./Data
def main():
    ...
    # Model
    model = Vgg16(
        num_classes=num_classes, input_shape=(224, 224, 3), C=args.C, kernel=args.kernel
    )
    ...
    svm_file = os.path.join(args.result, f"svm_layer_{tag}.sav")

    model.save(path=CNN_file, svm_path=svm_file)

    mlflow.log_artifact(CNN_file)
    mlflow.log_artifact(svm_file)

    print(f"Saved model weights in {CNN_file} and svm in {svm_file}")
