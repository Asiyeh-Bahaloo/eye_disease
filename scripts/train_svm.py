...
from eye.models.svm import SVM
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        dest="backbone",
        type=str,
        help="enter the backbone's name for feeding the results into SVM model",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of layers which need to be popped for feeding into SVM model",
    )
    parser.add_argument(
        "--C",
        dest="C",
        type=float,
        default=1,
        help="Regularization parameter of svm model",
        required=False,
    )
    parser.add_argument(
        "--kernel",
...
def main():
    ...
    if strtobool(args.imagenet_weights):
        svm.load_imagenet_weights()
        print("Imagenet weights loaded")

    if args.weights_path is not None:
        svm.backbone.load_weights(path=args.weights_path)
    ...
    if args.pre_epochs > 0:
        ...
        trainableParams = np.sum(
            [np.prod(v.get_shape()) for v in svm.backbone.model.trainable_weights]
        )
        nonTrainableParams = np.sum(
            [np.prod(v.get_shape()) for v in svm.backbone.model.non_trainable_weights]
        )
